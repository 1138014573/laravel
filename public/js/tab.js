...
$(function(){
	//头部交易中心；
	$('.nav li:eq(1)').css({'position':'relative'}).find('a').css({'width':'120px','text-align':'left','text-indent':'18px','background':'url(/img/huadownsj.png) 93px 32px no-repeat'})
	$('.nav li:eq(1) a').mouseover(function(){
		$(this).css({'background':'url(/img/huaupsj.png) #f4f4f4 93px 34px no-repeat'});
		$(this).next().show();
	})
	$('.nav li:eq(1)').mouseout(function(){
		$(this).find('a').css({'background':'url(/img/huadownsj.png) 93px 32px no-repeat'});
		$(this).find('.typeList').hide();
	})

/*
	$('.nav li:eq(2)').css({'position':'relative'}).find('a').css({'width':'120px','text-align':'left','text-indent':'18px','background':'url(/img/huadownsj.png) 93px 32px no-repeat'})
	$('.nav li:eq(2) a').mouseover(function(){
		$(this).css({'background':'url(/img/huaupsj.png) #f4f4f4 93px 34px no-repeat'});
		$(this).next().show();
	})
...
*/
	$('.typeList p').each(function(){
		$(this).mouseover(function(){
			...
			$(this).parent().show();
			$(this).parent().prev().css({'background':'url(/img/huaupsj.png) transparent 93px 34px no-repeat'});
		}).mouseout(function(){
			$(this).css({'color':'#666'});
			$(this).parent().hide();
		})
