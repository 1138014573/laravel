...
(function($, window, document, undefined) {
	...
	jRange.prototype = {
		...
		init: function(node, options) {
			...
			this.clickableBar  = this.domNode;
		...
		attachEvents: function() {
			this.clickableBar.parent().click($.proxy(this.barClicked, this));
		...
		setPosition: function(pointer, position, isPx, animate) {
			...
			var input_Value = Number(this.options.value);
			(input_Value > 100) ? (input_Value = 100.00) : input_Value;

			if( this.domNode.parent().parent().hasClass('rangebuy') ){
				if(input_Value > 0){
					$('#buy_number').val(input_Value.toFixed(2));
				}
				$('.rangebuy .pointBox .addpoint').each(function(){
					if( parseInt($(this).css('left')) <= highPos ){
						$(this).css({'background':'#de211d','border-color':'#de211d'});
					}else{
						$(this).css({'background':'#4d4d4d','border-color':'#4d4d4d'});
					}
				});
			}else{
				if(input_Value > 0){
					$('#sell_number').val(input_Value.toFixed(2));
				}
				$('.rangeSell .pointBox .addpoint').each(function(){
					if( parseInt($(this).css('left')) <= highPos ){
						$(this).css({'background':'#3dc18e','border-color':'#3dc18e'});
					}else{
						$(this).css({'background':'#4d4d4d','border-color':'#4d4d4d'});
					}
				});
