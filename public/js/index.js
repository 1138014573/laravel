 var reTime = 5000;
 var canvasReTime = 30000;
 btc_rmb_ajax = 10;
 /* var reTime = 1;
 var canvasReTime = 1;
 btc_rmb_ajax = 1; */ 

 var widthNumArr = { 'goc': 0.05, 'btc': 10, 'ltc': 0.1, 'lc': 0.005, 'lsk': 0.005, 'eth': 0.5, 'etc': 0.2, 'mtc': 0.002, 'uc': 0.01, 'lbc': 0.002, 'dsc': 0.002, 'lcc': 0.002, 'mac': 0.002, 'tur': 0.002, 'ecf': 0.002, 'osc': 0.002, 'gec': 0.002, 'hxi': 0.002, 'tge': 0.002, 'dstb': 0.002, 'mryc': 0.002, 'jnc': 0.002 , 'qtc': 0.002, 'jhp': 0.002, 'jjf': 0.002, 'rss': 0.002, 'npc': 0.002, 'bcc': 0.002, 'fdc': 0.002  };
 ...
 function btcsum(limit, coin, price_float, number_float, callType) {
     ...
     $.get('/json/' + coin + '_sum', function(data) {
         ...
         for (var type in data) {
             ...
             for (var i = 0, is = 0; i < d.lengths, is < ds.length; i++, is++) {
                 if (limit && i == limit) break;
                 //console.log(i + ':' + is);
                 if (i<d.length) {
                     var num = parseFloat(d[i].n);
                     var p = parseFloat(d[i].p);
                     var nums = parseFloat(ds[is].n);
                     var ps = parseFloat(ds[is].p);
                     var width = num * widthNumArr[coArr[0]] > 289 ? 289 : num * widthNumArr[coArr[0]];
                     idhtml = '<td class="' + (type == 'buy' ? 'buy' : 'sell') + '" style="width:50px;">' + (type == 'buy' ? '买' : '卖') + '(' + (parseInt(i) + 1) + ')</td>';
                     html += '<tr style="position:relative;">' + idhtml + '<td style="padding-left:5px; text-align:left;">￥' + parseFloat(d[i].p) + '</td><td style="padding-left:5px; text-align:left;">' + num + '</td><td style="width:' + width + 'px" style="border:0;" class="' + (type == 'buy' ? 'sell' : 'buy') + 'Span ' + (num != nums || p != ps ? 'animated flip' : 'noUpdate') + '"></td></tr>';
                 } else {
                    noUpdate(); break;
                 }
         ...
         if (data['sale'].length != 0) {
             ...
             if (user.length != 0) {
                 var buymax = formatfloat(user['cny_over'] / Number(data['sale'][0].p) - Math.pow(10, -number_float), number_float, 0);
                 if (buymax < 0) { buymax = 0.00; }
                 $('#buy_max').html(buymax);
         ...
         if (!callType) {
             setTimeout("btcsum(" + limit + ",'" + coin + "', " + price_float + ", " + number_float + ")", reTime);
         }
     }, 'json');
 ...
 /*最新成交信息*/
 function btcorder(limit, coin, display, price_float, number_float, callType) {
     $.get('/json/' + coin + '_order', function(d) {
         ...
         if (!callType) {
             setTimeout("btcorder(" + limit + ",'" + coin + "', '" + display + "', " + price_float + ", " + number_float + ")", reTime);
         }
     }, 'json');
 }
 ...
 /*我的委托信息*/
/*chen*/
 function mytrust(type, coin_from, price_float, number_float, callType)
 {

     $.ajax({
         type    : "GET",
         url     : '/ajax/mytrust/coin/' + coin_from + '/type/' + type,
         ...
         error   : function (XMLHttpRequest, textStatus, errorThrown)
         {
             ...
             {
                 mytrust(type, coin_from, price_float, number_float);
             }
         ...
         success : function (d)
         {
             ...
             if (!callType)
             {
                 setTimeout('mytrust(' + type + ', "' + coin_from + '", ' + price_float + ', ' + number_float + ')', reTime);
             }
         }
     });
 ...
 function ajaxtrustcancel(id, type, coin_from) { //chen
     $.get('/ajax/trustcancel/id/' + id + '/type/' + type + '/coin/' + coin_from, function(d) { //chen
         alert(d.msg);
         if (d.status) {
 ...
 function getCoinInfo(pairs) {
     $.get('/index/infofresh', { pairs: pairs }, function(data) {
         ...
         setTimeout("getCoinInfo('" + pairs + "')", 50000);
     }, "json");
 }
