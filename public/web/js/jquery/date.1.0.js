$(function() {
	$.fn.manhuaDate = function(options) {
		...
		//点击时间列表事件
		$("#calender td").off().on("click",function() {	
			var dv = $(this).html();
			if (dv != "&nbsp;"){
				 var str = "";
				 var month = $("#month").val();
				 ...
				 if (options.isTime){			
					var nd = new Date();
					str = $("#year").val() + options.fuhao + month + options.fuhao + dv + " "+ nd.getHours()+":"+nd.getMinutes()+":"+nd.getSeconds();
				 }else{
					str = $("#year").val() + options.fuhao + month + options.fuhao + dv;
				}				 
		...
		//文本框绑定事件
		$mhInput.on(options.Event,function(e){											
		...
		//当鼠标离开控件上面的时候延迟3秒关闭
		$(".calender").on("mouseleave",function(){ 
		...
		//当鼠标移到控件上面的时候显示
		$(".calender").on("mousemove",function(){     
			if(stc){
				clearTimeout(stc);//清除定时器
			}
			$(this).show();
